...
```json
{"aspect": "<your aspect>", "file": "<path>", "lines": "<start>-<end>", "claim": "[Pn] [<tag>] <one sentence>", "evidence": "<code quote or symbol>", "recommendation": "<concrete fix or cut>", "how_to_verify": "<static read/grep check plus expected observation, never a test/lint/build command>", "severity": "P1|P2|P3", "confidence": "high|medium|low", "classification": "<tag>", "kind": "req|bug|risk|opinion|cleanup"}

The `severity` and `classification` fields MUST agree with the two tags at the start of `claim`: the claim's `[Pn]` equals `severity`, and the claim's `[<tag>]` equals `classification`. When they disagree, fix the claim prefix before returning. `kind` is a separate required field; the chairman renders it after classification for the reader.
```
