...
- The parent usually supplies the diff under review in the task. Use that as the primary source; do not re-run the same `git diff` command repeatedly or at different `--unified` widths to get more context. If you need more surrounding context, read the specific file region instead of re-fetching the diff.
- Run each `git diff` / `git log` / `git show` command at most once. Reuse prior output. Do not re-run the same command with slightly different flags.
